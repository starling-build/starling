#!/usr/bin/env python3
"""Every composited frame from the right-click onward, side by side.
...
T0, N = 8, 12
SRC_MS = 1000.0 / 30.0
OURS_OFF, NAT_FIRST_OFF, NAT_SET_OFF = 2, 7, 8
...
d = ImageDraw.Draw(im)
d.text((40, 40), "Every composited frame after the right-click", font=f_h, fill=TEXT)
d.text((42, 96), "30 Hz panel — one frame every 33 ms. Nothing sampled: this is all of them.",
       font=f_ft, fill=DIM)

for r, (name, dirn, land, col) in enumerate([("Starling", "fr_ctx_ours", OURS_OFF, BLUE),
                                             ("Windows 11", "fr_ctx_native", NAT_SET_OFF, AMBER)]):
    y = TOP + r * (TH + 96)
    ...
    for i in range(N):
        ...
        if i == 0:
            d.text((x, y - 30), "click", font=f_n, fill=TEXT)
        if i == land:
...
d.text((42, Hd - 62),
       "Starling is finished in 2 frames — 67 ms.  Windows shows its first pixels at frame 7 and is complete at frame 8, 267 ms.",
       font=f_ft, fill=TEXT)
im.save("filmstrip-ctxmenu.png")
print("filmstrip-ctxmenu.png", im.size)
