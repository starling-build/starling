...
rc = sh.Run("cmd /c powershell -NoProfile -ExecutionPolicy Bypass -File C:\dist\capture-reference.ps1 > C:\dist\ref.log 2>&1", 0, True)
...
scp -q "$HERE/capture-reference.ps1" "$HOST:$REMOTE_DIR/capture-reference.ps1" \
